...
const resolver = new cross_chain_resolver_1.CrossChainResolver();
// Routes
// Health check
...
app.post('/api/swap', async (req, res) => {
    try {
        const { srcChain, dstChain, srcToken, dstToken, srcAmount, dstAmount, maker } = req.body;
        // Validate required fields
        if (!srcChain || !dstChain || !srcToken || !dstToken || !srcAmount || !dstAmount || !maker) {
            res.status(400).json({ error: 'Missing required fields' });
            return;
        }
        const swapState = await resolver.createSwap(srcChain, dstChain, srcToken, dstToken, BigInt(srcAmount), BigInt(dstAmount), maker);
        res.json({
...
app.post('/api/swap/eth-to-chain', async (req, res) => {
    try {
        ...
        const swapState = await resolver.createEthToChainSwap(dstChain, srcToken, dstToken, BigInt(srcAmount), BigInt(dstAmount), maker);
        res.json({
            orderId: swapState.order.orderId,
            status: swapState.status,
            secretHash: swapState.order.secretHash
...
app.post('/api/swap/chain-to-eth', async (req, res) => {
    try {
        ...
        const swapState = await resolver.createChainToEthSwap(srcChain, srcToken, dstToken, BigInt(srcAmount), BigInt(dstAmount), maker);
        res.json({
            orderId: swapState.order.orderId,
            status: swapState.status,
            secretHash: swapState.order.secretHash
...
app.post('/api/swap/partial', async (req, res) => {
    try {
        const { srcChain, dstChain, srcToken, dstToken, srcAmount, dstAmount, maker, partCount = 4 } = req.body;
        // Validate required fields
        if (!srcChain || !dstChain || !srcToken || !dstToken || !srcAmount || !dstAmount || !maker) {
            res.status(400).json({ error: 'Missing required fields' });
            return;
        ...
        const swapState = await resolver.createPartialSwap(srcChain, dstChain, srcToken, dstToken, BigInt(srcAmount), BigInt(dstAmount), maker, partCount);
        res.json({
            orderId: swapState.order.orderId,
            status: swapState.status,
            isPartialFill: true,
...
app.post('/api/demo/swap', async (req, res) => {
    try {
        const { srcChain = 'ethereum', dstChain = 'stellar', srcToken = '0x0000000000000000000000000000000000000000', // ETH
        dstToken = 'native', // XLM
