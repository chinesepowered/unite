 ...
 */
class BaseAdapter extends types_1.ChainAdapter {
    constructor(config, privateKey) {
        ...
        if (config.escrowFactory) {
            this.lopContract = new ethers_1.ethers.Contract(config.escrowFactory, LOP_ABI, this.wallet || this.provider);
        }
    }
    async createLimitOrder(order, side) {
    ...
    async fillLimitOrder(escrow, secret) {
        if (!this.wallet || !this.lopContract) {
            throw new Error('Wallet or LOP contract not configured');
        }
        try {
            ...
            const orderHash = await this.lopContract.hashOrder(limitOrder);
            const signature = await this.wallet.signMessage(ethers_1.ethers.getBytes(orderHash));
            // Fill the entire order
            const tx = await this.lopContract.fillContractOrder(limitOrder, signature, limitOrder.makingAmount, // Fill entire amount
    ...
    async checkOrderStatus(orderHash, maker) {
        if (!this.lopContract) {
