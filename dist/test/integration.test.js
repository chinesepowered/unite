...
const cross_chain_resolver_1 = require("../resolvers/cross-chain-resolver");
const types_1 = require("../core/types");
describe('Cross-Chain Integration Tests', () => {
    let resolver;
    beforeAll(() => {
        resolver = new cross_chain_resolver_1.CrossChainResolver();
    });
    describe('Basic Functionality', () => {
        ...
        it('should create a swap order', async () => {
            const swap = await resolver.createSwap('ethereum', 'stellar', '0x0000000000000000000000000000000000000000', // ETH
            'native', // XLM
            BigInt('1000000000000000000'), // 1 ETH
            BigInt('10000000'), // 1 XLM
            '0xe3B24b93C18eD1B7eEa9e07b3B03D03259f3942e');
            expect(swap.order.orderId).toBeDefined();
            expect(swap.status).toBe(types_1.SwapStatus.CREATED);
        ...
        it('should retrieve swap state', async () => {
            const swap = await resolver.createSwap('ethereum', 'sui', '0x0000000000000000000000000000000000000000', '0x2::sui::SUI', BigInt('1000000000000000000'), BigInt('1000000000'), '0xe3B24b93C18eD1B7eEa9e07b3B03D03259f3942e');
            ...
            expect(retrievedSwap?.status).toBe(types_1.SwapStatus.CREATED);
        });
        it('should create bidirectional swaps', async () => {
            // ETH to Chain
            const ethToChain = await resolver.createEthToChainSwap('tron', '0x0000000000000000000000000000000000000000', 'native', BigInt('1000000000000000000'), BigInt('1000000'), '0xe3B24b93C18eD1B7eEa9e07b3B03D03259f3942e');
            expect(ethToChain.order.srcChainId).toBe('ethereum');
            expect(ethToChain.order.dstChainId).toBe('tron');
            // Chain to ETH
            const chainToEth = await resolver.createChainToEthSwap('monad', 'native', '0x0000000000000000000000000000000000000000', BigInt('1000000000000000000'), BigInt('1000000000000000000'), '0xe3B24b93C18eD1B7eEa9e07b3B03D03259f3942e');
    ...
    describe('Swap Lifecycle', () => {
        it('should handle swap creation and status tracking', async () => {
            const swap = await resolver.createSwap('ethereum', 'stellar', '0x0000000000000000000000000000000000000000', 'native', BigInt('1000000000000000000'), BigInt('10000000'), '0xe3B24b93C18eD1B7eEa9e07b3B03D03259f3942e');
            // Initial state
            expect(swap.status).toBe(types_1.SwapStatus.CREATED);
        ...
        it.skip('should execute a full swap (requires testnet)', async () => {
            ...
            expect(finalState?.status).toBe(types_1.SwapStatus.COMPLETED);
        });
        it.skip('should handle swap cancellation (requires testnet)', async () => {
            ...
            expect(result.success).toBe(true);
            const finalState = resolver.getSwapState(swap.order.orderId);
            expect(finalState?.status).toBe(types_1.SwapStatus.CANCELLED);
        });
    });
    describe('Error Handling', () => {
        it('should reject unsupported chains', async () => {
            await expect(resolver.createSwap('unsupported-chain', 'ethereum', 'token1', 'token2', BigInt('1000'), BigInt('1000'), '0xe3B24b93C18eD1B7eEa9e07b3B03D03259f3942e')).rejects.toThrow('Unsupported chain');
        });
        ...
        it('should reject execution of non-existent swaps', async () => {
            await expect(resolver.executeSwap('0xinvalid')).rejects.toThrow('Swap 0xinvalid not found');
        });
    });
    describe('Multi-Chain Support', () => {
        ...
        testCases.forEach(({ src, dst }) => {
            it(`should support ${src} to ${dst} swaps`, async () => {
                const swap = await resolver.createSwap(src, dst, src === 'ethereum' ? '0x0000000000000000000000000000000000000000' : 'native', dst === 'ethereum' ? '0x0000000000000000000000000000000000000000' : 'native', BigInt('1000000000000000000'), BigInt('1000000000'), '0xe3B24b93C18eD1B7eEa9e07b3B03D03259f3942e');
