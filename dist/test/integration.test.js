...
describe('Cross-Chain Integration Tests', () => {
    ...
    describe('Basic Functionality', () => {
        ...
        it('should retrieve swap state', async () => {
            const swap = await resolver.createSwap('ethereum', 'sui', '0x0000000000000000000000000000000000000000', '0x2::sui::SUI', BigInt('1000000000000000000'), BigInt('1000000000'), '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266');
        ...
        it('should create bidirectional swaps', async () => {
            // ETH to Chain
            const ethToChain = await resolver.createEthToChainSwap('tron', '0x0000000000000000000000000000000000000000', 'native', BigInt('1000000000000000000'), BigInt('1000000'), '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266');
            expect(ethToChain.order.srcChainId).toBe('ethereum');
            expect(ethToChain.order.dstChainId).toBe('tron');
            // Chain to ETH
            const chainToEth = await resolver.createChainToEthSwap('monad', 'native', '0x0000000000000000000000000000000000000000', BigInt('1000000000000000000'), BigInt('1000000000000000000'), '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266');
    ...
    describe('Swap Lifecycle', () => {
        it('should handle swap creation and status tracking', async () => {
            const swap = await resolver.createSwap('ethereum', 'stellar', '0x0000000000000000000000000000000000000000', 'native', BigInt('1000000000000000000'), BigInt('10000000'), '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266');
            // Initial state
            expect(swap.status).toBe(types_1.SwapStatus.CREATED);
    ...
    describe('Error Handling', () => {
        it('should reject unsupported chains', async () => {
            await expect(resolver.createSwap('unsupported-chain', 'ethereum', 'token1', 'token2', BigInt('1000'), BigInt('1000'), '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266')).rejects.toThrow('Unsupported chain');
        });
    ...
    describe('Multi-Chain Support', () => {
        ...
        testCases.forEach(({ src, dst }) => {
            it(`should support ${src} to ${dst} swaps`, async () => {
                const swap = await resolver.createSwap(src, dst, src === 'ethereum' ? '0x0000000000000000000000000000000000000000' : 'native', dst === 'ethereum' ? '0x0000000000000000000000000000000000000000' : 'native', BigInt('1000000000000000000'), BigInt('1000000000'), '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266');
