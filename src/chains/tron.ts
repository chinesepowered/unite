...
export class TronAdapter extends ChainAdapter {
  ...
  async deployEscrow(order: SwapOrder, side: 'src' | 'dst'): Promise<EscrowDetails> {
    ...
    try {
      ...
      if (this.isTRX(tokenAddress)) {
        // Native TRX escrow
        const tx = await escrowContract.createHTLCEscrowTRX(
          order.secretHash,
          timelock,
          order.maker,
        ...
        const tx = await escrowContract.createHTLCEscrowTRC20(
          tokenAddress,
          amount,
          order.secretHash,
          timelock,
  ...
  private isTRX(tokenAddress: string): boolean {
    return !tokenAddress || tokenAddress === 'TRX' || tokenAddress === 'native';
  }

  private async getOrDeployEscrowContract(): Promise<any> {
  ...
  private extractEscrowIdFromReceipt(receipt: any): string {
    ...
    if (receipt.log && receipt.log.length > 0) {
      const log = receipt.log.find((l: any) => l.topics && l.topics[0] === 'EscrowCreated');
  ...
  private getEscrowContractSource(): string {
